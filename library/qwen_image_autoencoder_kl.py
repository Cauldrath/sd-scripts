...
# Convert standard keys to ComfyUI keys if necessary
def convert_to_comfyui_state_dict(sd):
    # Check if the state dict is in the standard format (indicated by the presence of quant_conv)
    if "quant_conv.bias" not in sd:
        return sd
    
    # Create a reversed key map: Standard -> ComfyUI
    ...
    for key in sd.keys():
        new_key = key
        key_without_suffix = key.rsplit(".", 1)[0]
        if key_without_suffix in reversed_key_map:
            new_key = key.replace(key_without_suffix, reversed_key_map[key_without_suffix])
        new_state_dict[new_key] = sd[key]
